...
template encrypt_card() {
    ...
    // Convert card to bits
    component cardBits = Num2Bits(8);
    cardBits.in <== card;
    ...
    // compute g ** card
    component gcard = EscalarMulFix(8, BASE8);
    for (var i = 0; i < 8; i ++) {
        gcard.e[i] <== cardBits.out[i];
    }
    var pt_x = gcard.out[0];
    var pt_y = gcard.out[1];
    ...
    pubKeyZ.p[0] <== pubKey_x;
    pubKeyZ.p[1] <== pubKey_y;

    // (pubKey ** z) * ptx
    component c2Adder = BabyAdd();
    c2Adder.x1 <== pubKeyZ.out[0];
    c2Adder.y1 <== pubKeyZ.out[1];
    c2Adder.x2 <== pt_x;
    c2Adder.y2 <== pt_y;
